...
*   If pod level resources are specified for a resource, that determines the overall footprint for the pod. Individual container-level requests, 
    including any additions from DRA claims, are not considered when calculating the pod-level footprint.
...
*   If `DRANodeAllocatableResources` is enabled and the pod's `status.nodeAllocatableResourceClaimStatuses` is populated:
    *   Iterate through each claim and obtain the node allocatable resource quantities allocated from the `direct` and `overhead` slices (`nodeAllocatableResourceClaimStatuses[].direct` and `nodeAllocatableResourceClaimStatuses[].overhead`).
    *   For each resource, the allocated direct and overhead quantities are added to the pod's total request.
...
*   The default preemption plugin simulates evicting ([`SelectVictimsOnNode()`](https://github.com/kubernetes/kubernetes/blob/451b50df783cc381f15c9c2a35d2948a699c249a/pkg/scheduler/framework/plugins/defaultpreemption/default_preemption.go#L252)) lower-priority pods. Because the victim pods are already running on the node, 
    and the pod status is populated with DRA allocations, the resource calculation helper function (`PodRequests()`) will accurately subtracts both the victim's Spec requests and its dynamic status claim allocations.
...
CPU quota enforcement (`cpu.max = -1`) at both the container and pod levels. This behavior is controlled by the feature gate `DisableCPUQuotaWithExclusiveCPUs` 
(default-on since 1.33) to prevent unexpected throttling (details in [Issue 70585](https://github.com/kubernetes/kubernetes/issues/70585)). 
...
1.  Allocated Resources (`pod.status.allocatedResources` and `pod.status.containerStatuses[*].allocatedResources`):
    *   No Change. Because Kubelet does not update cgroup requests based on DRA claims, these fields continue to
        be populated strictly based on standard requests in the PodSpec.
2.  Resources (`pod.status.resources` and `pod.status.containerStatuses[*].resources`):
    *   Requests: No change.
    *   Limits: Populated by including DRA allocations alongside standard Spec limits at both the container and
        pod levels (`pod.status.resources.limits` and `pod.status.containerStatuses[*].resources.limits`).
...
2.  **Allocated Resources**:
    ...
    *   Reported in the API through the `.status.containerStatuses[i].allocatedResources` field (allocated
        requests only).
...
-   **New Scheduler, Older Kubelet**: 
    - To proactively prevent pods utilizing DRA node allocatable resources from landing on older Kubelets that do not enforce 
      cgroup restriction based on DRA, we can use the [Node Declared Features framework](https://github.com/kubernetes/enhancements/tree/master/keps/sig-node/5328-node-declared-features).
    - We register a new declared feature `DRANodeAllocatableResources` in `node.status.declaredFeatures`.
      During scheduling, the scheduler will verify that pods with DRA node allocatable
